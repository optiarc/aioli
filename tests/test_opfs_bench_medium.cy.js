function runDataset(prefix) {
	const files = [
		`/home/lars/git/biowasm/derived-data/${prefix}-ref.fa`,
		`/home/lars/git/biowasm/derived-data/${prefix}-reads.fq.gz`,
		`/home/lars/git/biowasm/derived-data/${prefix}-sorted.bam`,
		`/home/lars/git/biowasm/derived-data/${prefix}-unsorted.bam`
	];

	cy.visit("/src/examples/opfs-bench-host.html?timeoutMinutes=30&resourceMinutes=1");
	cy.get("#source-mode").select("manual");
	cy.get("#files").selectFile(files, { force: true });
	cy.get("#run").click();
...
describe("OPFS benchmark medium dataset", () => {
